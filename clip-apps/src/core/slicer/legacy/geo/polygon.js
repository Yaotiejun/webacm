...
export class Polygon {
    ...
    toObject() {
        return {
            points: this.toArray(),
            inner: this.inner ? this.inner.map(i => i.toArray()) : undefined
        };
    }

    fromObject(obj) {
        this.fromArray(obj.points);
        this.inner = obj.inner ? obj.inner.map(a => newPolygon().fromArray(a)) : undefined
        return this;
    }
    ...
    noodle(width) {
        ...
        let ins = (clone.offset(width) !== undefined && clone.offset(width) !== null) ? clone.offset(width) : [];
    ...
    findArcCenters(opt = {}) {
        ...
        let tolerance = opt.tolerance || 1e-2,
            inside = opt.inside !== undefined && opt.inside !== null ? opt.inside : true,
    ...
    getZ(i) {
        ...
        return this.z !== undefined ? this.z : (p && p.z ? p.z : 0);
    }
    ...
    union(poly, min, all) {
        ...
        let fillang = this.fillang && this.area() > poly.area() ? this.fillang : poly.fillang,
            ...
            sp1 = this.toClipper(),
            sp2 = poly.toClipper(),
            minarea = min !== undefined && min !== null ? min : 0.1;
