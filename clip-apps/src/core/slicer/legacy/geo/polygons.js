...
export function union(polys, minarea, all, opt = {}) {
    ...
    if (opt.wasm && geo.wasm) {
        let min = minarea !== undefined && minarea !== null ? minarea : 0.01;
...
export function offset(polys, dist, opts = {}) {
    ...
    if (opts.wasm && geo.wasm) {
        ...
        for (let poly of polys) {
            ...
            poly = poly.toClipper();
            if (clean) poly = CleanPolygons(poly, (opts.cleanDist !== undefined && opts.cleanDist !== null) ? opts.cleanDist : config.clipperClean);
