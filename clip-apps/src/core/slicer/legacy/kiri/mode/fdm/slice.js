...
export function fdm_slice(settings, widget, onupdate, ondone) {
    ...
    if (!device.extruders || !device.extruders.length) {
        device.extruders = [{ extNozzle: process.sliceLineWidth || 0.4, extFilament: 1.75 }];
    }
    let maxtruder = Math.max(0, device.extruders.length - 1),
    ...
    slice(points, {
        ...
        ondone(err instanceof Error ? err : String(err));
    });
        ...
        if (isBrick) {
            ...
            for (let slice of slices) {
                ...
                if (slice.index === last) {
                    ...
                    nu.push(cap);
                    if (cap.tops) cap.tops.forEach((top, i) => {
...
function doRender(slice, isSynth, params, opt = {}) {
    ...
    slice.tops.forEach(top => {
        ...
        if (top.thin_fill && top.thin_fill.length) output
            .setLayer("thin fill", Color.thin)
            .addLines(top.thin_fill, vopt({ offset, height }));

        if (top.gaps && top.gaps.length && devel) output
...
export function projectFlats(slice, count, expand) {
    ...
    if (slice.flats && slice.flats.length) {
...
export function projectBridges(slice, count) {
    ...
    // these flats are marked for finishing print speed
    if (slice.bridges && slice.bridges.length) {
        slice.finishSolids = true;
        slice.isBridgeLayer = true;
    }
