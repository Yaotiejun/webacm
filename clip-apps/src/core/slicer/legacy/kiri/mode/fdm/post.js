...
function offset_default(params) {
    ...
    let test_flats = POLY.flatten(test_polys !== undefined && test_polys !== null ? test_polys : []);
...
function thin_type_3(params) {
    ...
    // console.log({ this: ((self.kiri_minion && self.kiri_minion.name) ? self.kiri_minion.name : self.kiri_worker), traces, polys });
...
function trace_noodle(noodle, noodleWidth, minR, midR, maxR, opt = {}) {
    ...
    for (let poly of noodle) {
        ...
        let inr = (poly.inner !== undefined && poly.inner !== null ? poly.inner : []).map(p => p.points.map(p => ({ x: p.x|0, y: p.y|0 })));
        ...
        for (let poly of insetp.flattenTo([])) {
            ...
            poly.segment(minR,  true).forEachSegment((p1, p2) => {
                ...
                for (let seg of segs) {
                    ...
                    if (int && int.dist < min.dist && int.dist < noodleWidth) {
                        min = int;
                        min.seg = seg;
                    }
                }
...
export function doTopShells(z, top, count, offset1, offsetN, fillOffset, opt = {}) {
    ...
    if (count) {
        ...
        if (offset1 === 0 && count === 1) {
            ...
            last = ret.last !== undefined && ret.last !== null ? ret.last : last;
            gaps = ret.gaps !== undefined && ret.gaps !== null ? ret.gaps : gaps;
        }
    } else {
