...
function legacyManifoldWasmUrl(path) {
    const name = (path || 'manifold.wasm').split('/').pop() || 'manifold.wasm';
    const base = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.BASE_URL) || '/';
    const root = base.endsWith('/') ? base : `${base}/`;
    if (typeof window !== 'undefined' && !/^https?:/i.test(root)) {
        return `${window.location.origin}${root}wasm/${name}`;
    }
    return `${root}wasm/${name}`;
}

let Instance;
manifold({
    locateFile(path) { return legacyManifoldWasmUrl(path); }
}).then(inst => {
        inst.setup();
        Instance = inst;
