...
motoClient.start(`/lib/mesh/pool.js?${version}`, motoClient.max() * 0);
...
function analyze(id, opt = {}) {
    ...
    let { geo, tool } = rec;
    if (!tool) {
        tool = rec.tool = new meshTool(opt);
    }
    if (tool.faces) {
...
const model = {
    ...
    subtract(recs) {
        ...
        let tools = recs.filter(rec => rec.tool)
            .map(rec => translate_encode(rec.id))
            .map(a => CSG.fromPositionArray(a));
        let diff = CSG.difference(...bases, ...tools);
        return CSG.toPositionArray(diff);
    },
    ...
    gen_gear(data, send) {
        ...
        const { gear, pitch } = new meshTool().generateGear(teeth, module, angle, offset);
        ...
        if (twist) {
            const rad = util.toRadians(twist);
    ...
    gen_threads(data, send) {
        ...
        height += zstep * 2;
        turns += 2;
        let verts = new meshTool().generateThreads(
            height,
            radius,
            turns,
            depth,
